    ...
    try:
        ...
        if len(pdgms) > 0:
            ...
            if generate_persistence_image:
                ...
                plt.tight_layout()
                plt.savefig(f'{base_path}/{variable_name}_{county_code}_peristence_diagram_auto.png')
                plt.close()



                #### PERSISTENXE DIAGRAM TEST
                ...
                # Increase the size of the points using the 's' parameter in scatter
                ax.scatter(births, deaths, s=100)  # 's' controls the size of the points

                # Define the range for the 45-degree line (from min to max of the plotted points)
                min_val = min(min(births), min(deaths))
                max_val = max(max(births), max(deaths))
                line_range = np.linspace(min_val, max_val, 100)

                # Plot the 45-degree line
                ax.plot(line_range, line_range, color='red', linestyle='--')  
                ...
                plt.tight_layout()
                plt.savefig(f'{base_path}/{variable_name}_{county_code}_peristence_diagram_manual.png')
                plt.close()
                ...
                plt.tight_layout()
                plt.savefig(f'{base_path}/{variable_name}_{county_code}_peristence_diagram_transformed_manual.png')
                plt.close()
